...
export default function ResendVerificationPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast({ title: 'Failed to send verification email. Please try again.', variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }
      ...
      <Card className="w-full max-w-md text-center">
        <CardContent className="pt-8 pb-8">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            If <strong>{email}</strong> has an unverified account, a fresh verification link is on its way.
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="text-center">
        ...
        <CardDescription>Enter the email you signed up with and we&apos;ll send a new link</CardDescription>
      </CardHeader>
      <CardContent>
