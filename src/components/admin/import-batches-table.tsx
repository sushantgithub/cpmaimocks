...
export function ImportBatchesTable({ batches }: { batches: Batch[] }) {
  ...
  async function remove(batch: Batch) {
    ...
    if (!confirm(`Delete import batch “${batch.name}” and exactly ${count} question${count === 1 ? '' : 's'}? This cannot be undone.`)) return
  ...
  return <div className="overflow-x-auto rounded-lg border">
    <table className="w-full text-sm">
      <thead className="border-b bg-gray-50"><tr>
        <th className="px-4 py-3 text-left">Batch</th><th className="px-4 py-3 text-left">Certification</th>
      ...
      <tbody className="divide-y">
        ...
        {batches.map((batch) => <tr key={batch.id}>
          <td className="px-4 py-3 font-medium">{batch.name}</td><td className="px-4 py-3">{batch.certification.name}</td>
